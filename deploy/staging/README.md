...
- Nginx-konfigurasjonen har Django-proxy for `/media/releases/`, `internal` delivery-location, `etag off`, `autoindex off`, `disable_symlinks on` og ingen shared `proxy_cache`; `etag off` gjelder bare den interne filhandleren og hindrer at Nginx erstatter Djangos checksum-ETag med filmetadata
