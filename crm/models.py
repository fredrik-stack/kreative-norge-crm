...
def slugify_norwegian(value: str, fallback: str) -> str:
    normalized = (
        (value or "")
        .replace("Æ", "AE")
        .replace("Ø", "O")
    ...
    return slugify(normalized) or fallback
...
class Tag(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify_norwegian(self.name, "tag")
            slug = base_slug
            suffix = 2
...
class InternalTag(models.Model):
    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE, related_name="internal_tags")
    name = models.CharField(max_length=64)
    slug = models.SlugField(max_length=80)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify_norwegian(self.name, "internal-tag")
            slug = base_slug
            suffix = 2
            while InternalTag.objects.filter(tenant=self.tenant, slug=slug).exclude(pk=self.pk).exists():
                slug = f"{base_slug}-{suffix}"
                suffix += 1
            self.slug = slug
        super().save(*args, **kwargs)
...
class Category(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify_norwegian(self.name, "category")
            slug = base_slug
            suffix = 2
...
class Subcategory(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base_slug = slugify_norwegian(self.name, "subcategory")
            slug = base_slug
            suffix = 2
...
class Person(models.Model):
    ...
    internal_tags = models.ManyToManyField(InternalTag, blank=True, related_name="persons")
