...
class InternalTagSerializer(serializers.ModelSerializer):
    def validate_name(self, value):
        tenant_id = self._get_effective_tenant_id()
        normalized = value.strip()
        if tenant_id is not None:
            qs = InternalTag.objects.filter(tenant_id=tenant_id, name=normalized)
            if self.instance is not None:
                qs = qs.exclude(pk=self.instance.pk)
            if qs.exists():
                raise serializers.ValidationError("Internal tag with this name already exists for this tenant.")
        return normalized
...
class OrganizationSerializer(serializers.ModelSerializer):
    ...
    internal_tags = InternalTagSerializer(many=True, read_only=True)
    internal_tag_ids = serializers.PrimaryKeyRelatedField(
        many=True,
        queryset=InternalTag.objects.all(),
        write_only=True,
        source="internal_tags",
        required=False,
    )
    ...
    def validate(self, attrs):
        ...
        tenant_id = self._get_effective_tenant_id()
        tags = attrs.get("tags")
        internal_tags = attrs.get("internal_tags")
        ...
        if tenant_id is not None and internal_tags is not None:
            invalid = [tag.name for tag in internal_tags if tag.tenant_id != tenant_id]
            if invalid:
                raise serializers.ValidationError(
                    {"internal_tag_ids": ["All internal tags must belong to the same tenant as the route."]}
                )
