...
class InternalTagViewSet(viewsets.ModelViewSet):
    permission_classes = [TenantScopedPermission]
    serializer_class = InternalTagSerializer

    def get_queryset(self):
        qs = InternalTag.objects.all().order_by("name")
        tenant_id = self.kwargs.get("tenant_id")
        if tenant_id is not None:
            qs = qs.filter(tenant_id=tenant_id)
        return qs
...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = (
            ...
            .order_by("name")
            .prefetch_related("org_people__person__contacts", "tags", "internal_tags", "subcategories__category")
        )
...
class PersonViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = Person.objects.all().order_by("full_name").prefetch_related("tags", "internal_tags", "subcategories__category")
        tenant_id = self.kwargs.get("tenant_id")
        if tenant_id is not None:
            qs = qs.filter(tenant_id=tenant_id)
        return qs
