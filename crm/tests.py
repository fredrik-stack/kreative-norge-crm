...
class PublicActorSiteTests(TestCase):
    ...
    def test_public_actor_templates_do_not_render_runtime_favicon_fallbacks(self):
    ...
    def test_public_actor_api_thumbnail_matches_stored_public_image(self):
        self.organization.og_image_url = fallback_preview_image(self.organization.website_url)
        self.organization.save(update_fields=["og_image_url"])
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIsNone(response.json()["thumbnail_image_url"])
        self.assertIsNone(response.json()["preview_image_url"])
