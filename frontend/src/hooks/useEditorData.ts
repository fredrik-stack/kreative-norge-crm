...
export function useEditorData() {
  ...
  const [organizationInternalTagInput, setOrganizationInternalTagInput] = useState("");
  const [personTagInput, setPersonTagInput] = useState("");
  const [personInternalTagInput, setPersonInternalTagInput] = useState("");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getPersons(tenantId),
      getTags(tenantId),
      getInternalTags(tenantId),
      getOrganizationPeople(tenantId),
    ])
      .then(([orgs, people, tenantTags, tenantInternalTags, links]) => {
        if (cancelled) return;
        setOrganizations(orgs);
        setPersons(people);
        setTags(tenantTags);
        setInternalTags(tenantInternalTags);
  ...
  useEffect(() => {
    if (selectedOrgId === "new") {
      ...
      setOrganizationInternalTagInput("");
      setSaveState("idle");
    ...
    setDraft({
      ...
      internal_tag_ids: (selectedOrganization.internal_tags ?? []).map((tag) => tag.id),
    ...
    setOrganizationTagInput(formatTagInput(selectedOrganization.tags ?? [], 5));
    setOrganizationInternalTagInput(formatTagInput(selectedOrganization.internal_tags ?? []));
    setSaveState("idle");
    setOrganizationFieldErrors({});
  ...
  useEffect(() => {
    ...
    setPersonDraft({
      ...
      internal_tag_ids: (selectedPerson.internal_tags ?? []).map((tag) => tag.id),
    ...
    setPersonTagInput(formatTagInput(selectedPerson.tags ?? [], 5));
    setPersonInternalTagInput(formatTagInput(selectedPerson.internal_tags ?? []));
    setContactDraft(emptyContactDraft);
    ...
    const baseline =
      selectedOrgId === "new"
        ...
        : selectedOrganization
          ...
          : emptyDraft;
    const baselineTagInput = formatTagInput(selectedOrganization?.tags ?? [], 5);
    const baselineInternalTagInput = formatTagInput(selectedOrganization?.internal_tags ?? []);
    return (
      !isEqualShallowOrganizationDraft(normalizeDraft(draft), normalizeDraft(baseline)) ||
      isDifferentTagInput(organizationTagInput, baselineTagInput, 5) ||
      isDifferentTagInput(organizationInternalTagInput, baselineInternalTagInput)
    );
  }, [draft, organizationInternalTagInput, organizationTagInput, selectedOrgId, selectedOrganization]);
    ...
    const baseline =
      selectedPersonId === "new"
        ...
        : selectedPerson
          ...
          : emptyPersonDraft;
    const baselineTagInput = formatTagInput(selectedPerson?.tags ?? [], 5);
    const baselineInternalTagInput = formatTagInput(selectedPerson?.internal_tags ?? []);
    const personDirty =
      !isEqualShallowPersonDraft(normalizePersonDraft(personDraft), normalizePersonDraft(baseline)) ||
      isDifferentTagInput(personTagInput, baselineTagInput, 5) ||
      isDifferentTagInput(personInternalTagInput, baselineInternalTagInput);
    const contactDirty =
  ...
  }, [contactDraft, personDraft, personInternalTagInput, personTagInput, selectedPerson, selectedPersonId]);
  ...
  }, [personDraft, personInternalTagInput, personTagInput, selectedPerson, selectedPersonId]);
  ...
  async function submitOrganizationDraft(): Promise<boolean> {
    ...
    setError(null);
    try {
      const [resolvedTagIds, resolvedInternalTagIds] = await Promise.all([
        resolveLabelIdsForInput({
          rawInput: organizationTagInput,
          listExisting: () => getTags(tenantId),
          createNew: (name) => createTag(tenantId, { name }),
          maxCount: 5,
          limitMessage: "Du kan legge inn maks 5 tags.",
        }),
        resolveLabelIdsForInput({
          rawInput: organizationInternalTagInput,
          listExisting: () => getInternalTags(tenantId),
          createNew: (name) => createInternalTag(tenantId, { name }),
        }),
      ]);
      const payload = normalizeDraft({
        ...draft,
        tag_ids: resolvedTagIds,
        internal_tag_ids: resolvedInternalTagIds,
      });
      if (selectedOrgId === "new") {
      ...
      setDraft((current) => ({ ...current, tag_ids: resolvedTagIds, internal_tag_ids: resolvedInternalTagIds }));
      const [nextTags, nextInternalTags] = await Promise.all([getTags(tenantId), getInternalTags(tenantId)]);
      setTags(nextTags);
      setInternalTags(nextInternalTags);
      setOrganizationTagInput(normalizeTagInput(organizationTagInput, 5));
      setOrganizationInternalTagInput(normalizeTagInput(organizationInternalTagInput));
      setSaveState("saved");
  ...
  async function quickCreateOrganization(draftOverride: OrganizationPatch): Promise<Organization> {
    if (!tenantId) {
      throw new Error("Velg en tenant før du oppretter en aktør.");
    }
    const payload = normalizeDraft(draftOverride);
    const nextErrors = validateOrganizationDraft(payload);
    if (Object.keys(nextErrors).length > 0) {
      throw new Error("Rett feltene markert i rødt før du lagrer aktøren.");
    }
    try {
      const created = await createOrganization(tenantId, payload);
      await reloadOrganizations(created.id);
      ...
      throw new Error(apiErrorMessage(error, "Kunne ikke opprette aktøren."));
    }
  }
  ...
  async function submitPersonDraft(): Promise<boolean> {
    ...
    try {
      const [resolvedTagIds, resolvedInternalTagIds] = await Promise.all([
        ...
        resolveLabelIdsForInput({
          rawInput: personInternalTagInput,
          listExisting: () => getInternalTags(tenantId),
          createNew: (name) => createInternalTag(tenantId, { name }),
        }),
      ]);
      ...
      setPersonDraft((current) => ({ ...current, tag_ids: resolvedTagIds, internal_tag_ids: resolvedInternalTagIds }));
      const [nextTags, nextInternalTags] = await Promise.all([getTags(tenantId), getInternalTags(tenantId)]);
      setTags(nextTags);
      setInternalTags(nextInternalTags);
      setPersonTagInput(normalizeTagInput(personTagInput, 5));
      setPersonInternalTagInput(normalizeTagInput(personInternalTagInput));
  ...
  return {
    ...
    setOrganizationTagInput,
    organizationInternalTagInput,
    setOrganizationInternalTagInput,
    personDraft,
    setPersonDraft,
    personTagInput,
    setPersonTagInput,
    personInternalTagInput,
    setPersonInternalTagInput,
    overviewQuery,
    setOverviewQuery,
...
function normalizeDraft(draft: OrganizationPatch): OrganizationPatch {
  return {
    ...
    internal_tag_ids: uniqueSortedIds(draft.internal_tag_ids),
...
function isEqualShallowOrganizationDraft(a: OrganizationPatch, b: OrganizationPatch): boolean {
  return (
    ...
    isEqualIdList(a.internal_tag_ids, b.internal_tag_ids) &&
...
async function resolveLabelIdsForInput<T extends { id: number; name: string }>(input: {
  rawInput: string;
  listExisting: () => Promise<T[]>;
  createNew: (name: string) => Promise<T>;
  maxCount?: number;
  limitMessage?: string;
}): Promise<number[]> {
  const { rawInput, listExisting, createNew, maxCount, limitMessage } = input;
  const uniqueNames = parseTagNames(rawInput);
  if (maxCount && uniqueNames.length > maxCount) {
    throw new Error(limitMessage ?? `Du kan legge inn maks ${maxCount} tags.`);
  }
  const names = typeof maxCount === "number" ? uniqueNames.slice(0, maxCount) : uniqueNames;
  ...
  for (const name of names) {
    ...
    const created = await createNew(name);
...
function normalizeTagInput(value: string, maxCount?: number): string {
  const names = parseTagNames(value);
  return (typeof maxCount === "number" ? names.slice(0, maxCount) : names).join(", ");
}

function formatTagInput(tags: Array<{ name: string }>, maxCount?: number): string {
  return normalizeTagInput(tags.map((tag) => tag.name).join(", "), maxCount);
}

function isDifferentTagInput(a: string, b: string, maxCount?: number): boolean {
  return normalizeTagSetInput(a, maxCount) !== normalizeTagSetInput(b, maxCount);
}

function normalizeTagSetInput(value: string, maxCount?: number): string {
  const names = parseTagNames(value);
  return (typeof maxCount === "number" ? names.slice(0, maxCount) : names)
