...
export async function getInternalTags(tenantId: number): Promise<InternalTag[]> {
  const data = await request<InternalTag[] | Paginated<InternalTag>>(`/api/tenants/${tenantId}/internal-tags/`);
  return isPaginated(data) ? data.results : data;
}

export async function createInternalTag(
  tenantId: number,
  payload: Pick<InternalTag, "name">,
): Promise<InternalTag> {
  return request<InternalTag>(`/api/tenants/${tenantId}/internal-tags/`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
