...
let internalTagsByTenantState: Record<number, InternalTag[]> = {};
...
export function resetMockEditorData() {
  ...
  internalTagsByTenantState = {
    1: [
      { id: 400, tenant: 1, name: "Prioritet", slug: "prioritet", created_at: "2026-01-01T00:00:00Z" },
      { id: 401, tenant: 1, name: "Partner", slug: "partner", created_at: "2026-01-01T00:00:00Z" },
    ],
  };
...
export const handlers = [
  ...
  http.get("/api/tenants/:tenantId/internal-tags/", ({ params }) => {
    const tenantId = Number(params.tenantId);
    return HttpResponse.json(internalTagsByTenantState[tenantId] ?? []);
  }),
  ...
  http.post("/api/tenants/:tenantId/internal-tags/", async ({ params, request }) => {
    const tenantId = Number(params.tenantId);
    const body = (await request.json()) as { name?: string };
    const current = internalTagsByTenantState[tenantId] ?? [];
    const normalized = (body.name ?? "").trim();
    const existing = current.find((tag) => tag.name.toLowerCase() === normalized.toLowerCase());
    if (existing) {
      return HttpResponse.json(existing);
    ...
    internalTagsByTenantState[tenantId] = [...current, created];
    return HttpResponse.json(created, { status: 201 });
  }),
  ...
  http.post("/api/tenants/:tenantId/import-jobs/:jobId/preview/", ({ params }) => {
    ...
    const actorRow: ImportRow = {
      ...
      raw_payload_json: {
        ...
        organization_tags: "",
        organization_internal_tags: "Prioritet",
        organization_website_url: "",
        person_full_name: job.import_mode === "ORGANIZATIONS_ONLY" ? "" : "Ada Editor",
        person_internal_tags: job.import_mode === "ORGANIZATIONS_ONLY" ? "" : "Partner",
      },
      normalized_payload_json: {
        organization: { name: "Kreativ Demo AS", tags: ["Etablert"], internal_tags: ["Prioritet"], categories: [], subcategories: [] },
        person: { full_name: job.import_mode === "ORGANIZATIONS_ONLY" ? "" : "Ada Editor", tags: ["Turné"], internal_tags: ["Partner"] },
      },
