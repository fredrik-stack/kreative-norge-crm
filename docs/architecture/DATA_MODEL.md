...
Interne keys er tenant-scopede og deterministiske, men er ikke public release keys. `image_renditions_public` brukes i denne fasen bare som artifact-store; `releases/<release_uuid>/<variant>.<ext>` materialiseres ikke. Fase 3D.1 og 3D.2 kobler den eksisterende ingest- og selectiontjenesten til en intern Editor/API-flyt, men bare bak det fortsatt avslåtte kode-default-flagget. 3D.2 er foreløpig bare implementert og lokalt backendtestet på featurebranch; CI, staging og eiergodkjenning gjenstår. Public storage reservation, projection og serving er fortsatt ikke implementert, og legacybildeflyten er uendret.
