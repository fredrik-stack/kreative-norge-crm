...
Interne keys er tenant-scopede og deterministiske, men er ikke public release keys. `image_renditions_public` brukes i denne fasen bare som artifact-store; `releases/<release_uuid>/<variant>.<ext>` materialiseres ikke. Fase 3D.1 og 3D.2 kobler den eksisterende ingest- og selectiontjenesten til en intern Editor/API-flyt, men bare bak det fortsatt avslåtte kode-default-flagget. Precision/zoom-oppfølgingen er fullverifisert lokalt, CI-grønn, live Brave-verifisert og visuelt eiergodkjent i staging. Public storage reservation, projection og serving er fortsatt ikke implementert, og legacybildeflyten er uendret.
...
Samme SQLite holder avledet `release_state`, permanent keyindeks og read-cursor som bygges deterministisk på nytt fra eventene. `retired` og `denied` er terminalt for release-ID-en; republisering bruker ny UUID/key. Reservationen binder tenant-/Organization-/selection-/revisjon-/rendition-sett-ID-er og de tre artifact key/checksum-snapshottene, mens public keys genereres av samme rene builder som Django-tjenesten. Databaseaggregatet fryser fortsatt historisk mapping, men bestemmer ikke alene om releasen er aktiv eller leverbar. Dagens `create_organization_image_release()` er uendret DB-only; binding til en ankret reservation kommer i 3E.1B.
...
`image_renditions_public` forblir intern artifact-storage. Materialiserte `releases/<release_uuid>/<variant>.<ext>` skal ligge i eget `/srv/kreative-norge/media/public-delivery/` med separat cleanup-/purgeeierskap. 3E.1B innfører ingen automatisk releasefilsletting, og delivery-rooten skal ikke inn i dagens generiske orphan-cleanup. Dagens release-tjeneste utfører ingen av disse runtimeovergangene og mangler fortsatt entydig idempotency-håndheving per selection-revisjon; eksakt ledger-/databasehåndheving er et implementeringsspørsmål, ikke en besluttet lifecycle-kolonne.
