...
## Public runtime safety-state – fase 3E.1A–3E.3 ACTIVE/verifisert i staging

[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) skiller public runtimeens sikkerhetstilstand fra PostgreSQL-aggregatet. `image_safety` kan lese aktiv staging schema v1 og har en eksplisitt, transaksjonell upgrade til schema v2 uten å omskrive v1-events, sekvenser, hashes, receipts, ledger-ID eller cursor. V2 bruker en separat append-only eventtabell etter den fryste v1-grensen og rebuildbare read-models for tenant/checksum-deny og tenant/Organization-legacyguard. Før første v2-event er ankerbundle fortsatt byteidentisk v1; første v2-event gir et versjonert v2-bundle. Staging forblir schema v1 frem til separat upgradegate. Controlled serving, projection, target-API og PUBLIC/head leser state gjennom `authorize`; approval/restore/release bruker `check_checksum`, og legacyresolverne bruker `legacy_guard` fail-closed når materialiseringsruntimeen er aktiv.
