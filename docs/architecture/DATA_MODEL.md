...
Interne keys er tenant-scopede og deterministiske, men er ikke public release keys. `image_renditions_public` brukes i denne fasen bare som artifact-store; `releases/<release_uuid>/<variant>.<ext>` materialiseres ikke. Fase 3D.1 og 3D.2 kobler den eksisterende ingest- og selectiontjenesten til en intern Editor/API-flyt, men bare bak det fortsatt avslåtte kode-default-flagget. Første 3D.2-versjon er teknisk stagingverifisert; precision/zoom-oppfølgingen er lokalt målrettet testet og venter på full CI, ny stagingverifisering og visuell eierretest. Public storage reservation, projection og serving er fortsatt ikke implementert, og legacybildeflyten er uendret.
