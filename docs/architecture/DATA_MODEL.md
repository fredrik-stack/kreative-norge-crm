...
- `ImageReviewEvent` lagrer append-only snapshots av første selection-låsing, eksplisitt replacement eller ordinær fjerning til systemfallback, med nullable `SET_NULL`-referanser til levende domeneobjekter
...
`clean()` avviser også tenant-mismatch mot organisasjon/rendition-sett og whitespace-only tekst. Vanlige ForeignKeys gir fortsatt ikke en komplett cross-tenant-databasegaranti. `lock_organization_image_selection` og `remove_organization_image_to_fallback` er de eneste godkjente skriverutene: begge er feature-gated, håndhever tenant og capability, låser `Organization` som serialiseringspunkt, kontrollerer `expected_revision`, arkiverer bare statusen på forrige aktive rad og oppretter ny aktiv selection og event i én transaksjon. Bare den spesialiserte remove-kommandoen kan utføre asset → systemfallback; den generiske lock-kommandoen avviser denne overgangen og fallback → fallback.

`ImageReviewEvent` bruker snapshots som autoritativ historisk identitet, slik at constraints ikke avhenger av at nullable live-referanser fortsatt finnes. Asset-events lagrer intern, versjonert godkjenningstekst og begrenset proveniens; fallback-events lagrer ingen falsk rettighetsgodkjenning. `selection_removed_to_fallback` krever tidligere selection-ID og revisjon i databaseconstraintet, mens domenekommandoen håndhever at den tidligere aktive selectionen faktisk var et asset. Både standard- og base manager blokkerer ordinær update, delete, bulk-update og upsert på applikasjons-/ORM-nivå. Base manager tillater bare lesing, nye inserts og eksakt nullstilling av de nullable live-referansene som Django trenger for `SET_NULL`; tenant-`CASCADE` er bevart. Dette er ikke kryptografisk WORM eller en absolutt databasegaranti. Når nye append-only removal-events senere finnes i et aktivt miljø, skal rollback bruke feature-off eller en ny fremoverrettet migrasjon, aldri sletting eller omskriving av eventhistorikk.
...
OrganizationImageSelection
    → append-only ImageReviewEvent for locking/replacement/removal-to-fallback
```

Første låsing, replacement og ordinær fjerning til fallback har append-only bildehistorikk. Kandidat-, takedown-, restore- og retentionevents kommer i separate leveranser. Assetet eies av tenant og kan finnes før en aktør. Den implementerte typed selection-modellen gjelder bare `Organization`, uten `GenericForeignKey` eller en generell selection for andre objekttyper.
