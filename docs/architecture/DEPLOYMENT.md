...
Fase 3D.2 legger til Brave-bildesøk som en valgfri intern kilde. `BRAVE_IMAGE_SEARCH_API_KEY` leses bare av Django fra servermiljøet og har tom standardverdi. Staging-Compose bruker repo-rotens ignorerte `/srv/kreative-norge-crm/.env.staging` som `env_file` for `api`. En gyldig nøkkel skal installeres der direkte på serveren av prosjekteier/operatør, aldri i Git, chat, PR, `.env.staging.example`, `VITE_`-variabel eller frontend-buildargument. Deretter rekreeres bare API-containeren, og kontrollen skal bare rapportere `configured`/`missing`, aldri verdien. Nøkkelen skal aldri eksponeres til frontend, API-responser eller logger. Manglende nøkkel gir kontrollert `503` med kode `brave_not_configured`. Sikker kontroll 2026-08-11 fant ingen lokal eller stagingcredential, så live providerrespons er ikke verifisert. Gjeldende standardvilkår punkt 3(b) krever fortsatt at avtaleeier sikrer skriftlige sluttbrukerforpliktelser; dette løses operativt og ikke med en egen samtykkemotor i bilde-API-et.
