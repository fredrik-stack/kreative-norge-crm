...
Kun canonical `GET`/`HEAD /media/releases/<uuidv4>/<square|landscape|share>.<webp|png|jpg>` kan nå Django-gaten. Den krever publisert Organization, gjeldende aktiv asset-selection, eksakt immutable DB-mapping, positiv metadata, read-only ledger-autorisasjon og byte-/checksum-/format-/dimensjonsverifikasjon av alle tre materialiserte filer for hver request. Ikke-canonical former under samme prefix treffer en tom `404/no-store`-catch-all uten domenekall. Den canonical viewen er CSRF-unntatt bare for at dens write-frie metodegate skal gi `405/no-store` for alt annet enn GET/HEAD før domenekall. Avvist eller ukjent release gir `404`; safety-/bridge-/filfeil gir `503`. Begge feiltyper har `Cache-Control: no-store`. Godkjent svar bruker bare intern `X-Accel-Redirect`, `ETag` fra checksum og `Cache-Control: private, max-age=60, must-revalidate`; validert `If-None-Match` gir `304` først etter de samme gatene. Den interne Nginx-locationen har `etag off` og setter `ETag` eksplisitt fra `$upstream_http_etag`, slik at Djangos checksum-ETag bevares i stedet for å bli fjernet eller erstattet av en filmetadata-ETag. Shared proxycache og `immutable` brukes ikke. Loggeren `crm.public_image_serving` har en eksplisitt INFO-consolehandler, slik at de strukturerte requestutfallene faktisk inngår i containerloggen; formatteringen og feltene utelater filesystempath, credentials og ledgerhistorikk.

Den separate [aktiveringsgaten](../status/STAGING_PHASE_3E1C_ACTIVATION_2026-08-24.md) beviste alle tre varianter, checksum-`ETag`, HEAD/304, tomme 404/405/503-svar, bridge- og filfeil, origins, logger, mountisolasjon, restart og backup/restore. 3E.2 projection/API-shadow er senere [stagingverifisert](../status/STAGING_PHASE_3E2_SHADOW_2026-08-24.md), [3E.3-stagingaktivering og rollback](../status/STAGING_PHASE_3E3_CUTOVER_2026-08-24.md) er grønn, og [3E.4-staginggaten](../status/STAGING_PHASE_3E4_TAKEDOWN_2026-08-24.md) har aktivert ledger v2 og bevist permanent deny/restore/republish. 3E.4 har fortsatt write-only kode-/eksempelstandard `PUBLIC_IMAGE_TAKEDOWN_ENABLED=False`. Etter første ekte deny kan write-gaten slås av, men ledger/checksum/legacy/serving-enforcement må forbli aktiv; rollback er forward-only og må aldri downgrade eller omskrive safetyevents.
