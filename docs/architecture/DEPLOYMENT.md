...
## Fase 3E.1B – foundation deployet og verifisert, materialisering av
...
Gunicorn har 60 sekunders worker-timeout. Bridge-serveren har 45 sekunders operation deadline, Django-klienten har 50 sekunder og framingtimeouten er 5 sekunder. Staginggaten 2026-08-23 verifiserte Linux/Docker `SO_PEERCRED`, socket `root:root` `0600`, API-/webisolasjon, delivery-persistens gjennom API-recreate og identisk delivery-checksum i ny backup og isolert restore. `PUBLIC_IMAGE_RELEASE_MATERIALIZATION_ENABLED` forblir likevel `False`; faktisk syntetisk reserve/materialize/retry/activate er neste activation-gate. Se [stagingevidensen](../status/STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md).
