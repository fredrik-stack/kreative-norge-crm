...
S3, AWS, Backblaze, CDN og flerleverandørløsning er utsatt til dokumentert vekst- eller driftsbehov. Fase 3B.3-A etablerte release-aggregatet. Fase 3E.1A har implementert og stagingaktivert permanent reservation-/lifecycle-ledger, rebuildbar read-model, incident restore og host/systemd Borg-anchor. Fase 3E.1B har flyttet UUID/key-valget til atomisk host-reservasjon, lagt unik DB-binding, separat delivery-root, create-only/no-clobber, read-back og activation. Materialisering er `ACTIVE` i staging etter syntetisk crash/restart/retry-gate. Fase 3E.1C-controlled serving, 3E.2 projection-shadow og 3E.3 target-schema/PUBLIC/head er `ACTIVE` i staging; 3E.4 runtime-deny er implementert default-off og venter på separat ledgerupgrade/aktivering.
...
Den separate [aktiveringsgaten](../status/STAGING_PHASE_3E1C_ACTIVATION_2026-08-24.md) beviste alle tre varianter, checksum-`ETag`, HEAD/304, tomme 404/405/503-svar, bridge- og filfeil, origins, logger, mountisolasjon, restart og backup/restore. 3E.2 projection/API-shadow er senere [stagingverifisert](../status/STAGING_PHASE_3E2_SHADOW_2026-08-24.md), og [3E.3-stagingaktivering og rollback](../status/STAGING_PHASE_3E3_CUTOVER_2026-08-24.md) er grønne. 3E.4 legger til write-only `PUBLIC_IMAGE_TAKEDOWN_ENABLED=False`, ledger-v2-upgradekommando og en deny-first API/service. Etter første ekte deny kan write-gaten slås av, men ledger/checksum/legacy/serving-enforcement må forbli aktiv; rollback er forward-only og må aldri downgrade eller omskrive safetyevents.
