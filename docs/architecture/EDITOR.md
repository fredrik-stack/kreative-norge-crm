...
Nær søkefeltet opplyser Editor at søket utføres via Brave, at teksten kan lagres hos provider i opptil 90 dager og at sensitiv eller intern informasjon ikke skal skrives inn. Nær resultatene står den separate rettighetspåminnelsen om å kontrollere at bildet kan brukes. Gjeldende standardvilkår punkt 3(b) krever fortsatt at avtaleeier sikrer skriftlige sluttbrukerforpliktelser; dette er en operativ avtaleplikt, ikke en egen samtykkemotor i Editor. Staging har nå en server-side credential, og den endelige live-gaten returnerte 30 ekte kandidater for den foreslåtte queryen gjennom den aktive `search_lang=nb`-kontrakten uten å eksponere credentialen.
