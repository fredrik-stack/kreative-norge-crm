...
Foto har fokusforvalgene Venstre/Midt/Høyre og Topp/Midt/Bunn som snarveier til samme presise X/Y-state som sliderne under den kollapsede `Finjuster utsnitt`. Zoom går fra 100 til 300 prosent; 100 prosent er maksimalt cover-utsnitt uten tom flate. `Tilbakestill utsnitt` setter sentrum/sentrum/100 prosent. Alle tre live-previewene bruker samme kantklampede coverberegning som serverprocessing per variant og oppdateres uten nettverkskall per slidersteg. Etter `Prosesser valgt bilde` er serverens faktiske `square`-, `landscape`- og `share`-renditions fasit før godkjenning. Logo bruker contain, viser et helt-motiv-preview og skjuler/avviser fokus og zoom.
