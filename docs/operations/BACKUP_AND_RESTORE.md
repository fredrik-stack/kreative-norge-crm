...
Fase 3F gjennomførte 2026-08-25 en ny predeploy-backup før migrasjon `0031` og en separat postdeploy-backup/isolert restore etter typed Import-verifikasjon. Restorekontrollen fulgte `ImportRow → ImportImageDecision → ImageReviewEvent → OrganizationImageSelection → ImageAsset/ImageRenditionSet`, kontrollerte private originals/artifacts og public delivery sammen og revaliderte safety-ledger `READY`. En eldre database eller mediekopi kunne ikke oppheve nyere release-/checksum-deny. Orphan-verifikasjonen var dry-run-only med null eligible og null slettet; fysisk feltdropp, assetcleanup, generisk retensjon og providerpurge er egne senere gater. Før første typed beslutning kan den additive migrasjonen reverseres; etter første write er rollback `IMPORT_IMAGE_DECISIONS_ENABLED=False` og fremoverrettet retting, ikke sletting av beslutning eller audit. Se [3F-evidensen](../status/STAGING_PHASE_3F_LEGACY_IMPORT_2026-08-25.md).
