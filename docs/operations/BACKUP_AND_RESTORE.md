...
Fase 3E.1B har `/srv/kreative-norge/media/public-delivery/` eksplisitt i den aktive `HOST_MEDIA_PATHS`-allowlisten og compose-/storagefoundation. Området er opprettet `root:root` mode `0750`, API-mountet, holdt utenfor web og backup-/restore-verifisert i staging. En representativ ueksponert probe overlevde API-recreate, ny off-server backup og isolert restore med identisk checksum før den ble fjernet fra live root. `PUBLIC_IMAGE_RELEASE_MATERIALIZATION_ENABLED` forblir `False`, og dette aktiverer fortsatt ikke public serving. Se [datert evidens](../status/STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md).
