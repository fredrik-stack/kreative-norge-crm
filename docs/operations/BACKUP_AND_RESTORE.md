...
Fase 3E.1B har lagt `/srv/kreative-norge/media/public-delivery/` eksplisitt i repositoryets `HOST_MEDIA_PATHS`-allowlist og compose-/storagefoundation. Området er fortsatt ikke opprettet, tatt inn i aktiv serverkonfigurasjon eller backup-/restore-verifisert i staging. Før materialiseringsflagget kan aktiveres må hostpathen opprettes kontrollert, den aktive root-only backupkonfigurasjonen oppdateres, og en representativ delivery-fil må overleve backup og isolert restore. Dette aktiverer fortsatt ikke public serving.
