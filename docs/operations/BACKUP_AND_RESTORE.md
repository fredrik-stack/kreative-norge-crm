...
Fase 3F krever en ny predeploy-backup før migrasjon `0031` og en separat isolert restore etter typed Import-verifikasjon. Restorekontrollen skal følge `ImportRow → ImportImageDecision → ImageReviewEvent → OrganizationImageSelection → ImageAsset/ImageRenditionSet`, kontrollere private originals/artifacts og public delivery sammen og deretter revalidere safety-ledger `READY` før public runtime åpnes. En eldre database eller mediekopi kan ikke oppheve nyere release-/checksum-deny. Orphan-verifikasjonen er dry-run-only i denne fasen; fysisk feltdropp, assetcleanup, generisk retensjon og providerpurge er egne senere gater. Før første typed beslutning kan den additive migrasjonen reverseres; etter første write er rollback `IMPORT_IMAGE_DECISIONS_ENABLED=False` og fremoverrettet retting, ikke sletting av beslutning eller audit.
