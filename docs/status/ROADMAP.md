...
Fase 3 er avsluttet og etablerer bare bildekontrakten som senere Import 2.0 skal bruke. Fase 4 etablerer internasjonal telefonidentitet før full Import 2.0. Produkt- og UX-design for Import 2.0 ligger deretter i fase 5, og full implementering ligger i fase 7. Fase 4A er godkjent og merget med [ADR-010](../decisions/ADR-010-INTERNATIONAL_PHONE_IDENTITY_AND_NORMALIZATION.md) via PR #56. Fase 4B er `READY_FOR_4C` etter [skrivebeskyttet stagingbaseline](STAGING_PHASE_4B_PHONE_BASELINE_2026-08-25.md). Fase 4C har implementert det isolerte normaliseringsdomenet; stagingverifikasjon gjenstår før `READY_FOR_4D`.
...
**Status:** 4A er godkjent og merget med [ADR-010](../decisions/ADR-010-INTERNATIONAL_PHONE_IDENTITY_AND_NORMALIZATION.md) via PR #56. 4B er gjennomført skrivebeskyttet og klassifisert `READY_FOR_4C`. 4C-normaliseringsdomenet er implementert med stagingverifikasjon pending; ingen modell- eller dataendring er utført.
...
3. **4C – Normalization domain (`IMPLEMENTED / STAGING PENDING`):** `phonenumbers==9.0.37` bak én ren felles telefonadapter for parsing, eksplisitt regionkontekst, validering, typed resultat og E.164. Ingen caller er koblet til ennå; se [domenekontrakten](../architecture/PHONE_NORMALIZATION.md).
...
Fase 4 kan først markeres `CLOSED / VERIFIED` etter separat implementasjon og stagingverifikasjon i 4B–4H. ADR-010- og 4B-gatene er oppfylt. 4C-koden er implementert isolert; separat stagingevidens må være grønn før neste oppgave kan være planlegging av fase 4D.
