...
**Status:** Fase 1 og 2 gjennomført; fase 3A–3D.2 gjennomført som tidligere dokumentert; lokal Hetzner storage-/backup-MVP og ADR-009 fase 3E.1A safety-ledger/off-server anchor er **ACTIVE** i staging; fase 3E.1B-materialisering er **ACTIVE / VERIFIED** i staging med én permanent syntetisk release; fase 3E.1C-serving er implementert bak et separat default-off flagg og venter på livegate; fase 3E.2–3E.4 og PUBLIC-kobling er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere; ingen offentlig bildebruk

**Teknisk sist verifisert:** 2026-08-23
...
Den separate [3E.1B-materialiseringsaktiveringen 2026-08-23](STAGING_PHASE_3E1B_MATERIALIZATION_ACTIVATION_2026-08-23.md) ble gjennomført fra `main` på merge `ee42c8217a7d32321be7d35ecfb368a280e6df82` etter grønn main-CI-run `32646334974`, 25 målrettede lokale tester og pre-activation-backup `kreative-norge-staging-20260823T185358Z`. Gaten beviste hard no-clobber, permanent reserve og DB-binding, kontrollert crash etter første fil, API-restart, read-back, fullføring, activation og ny idempotent retry uten overskriving eller duplikate ledger-events. Staging har nå én aktiv syntetisk release, tre persistente delivery-filer og `READY` safety-ledger på cursor `5`; post-activation-backup `kreative-norge-staging-20260823T185950Z` er fullverifisert og isolert restore-verifisert. Serving, projection, API/PUBLIC og legacy-cutover er fortsatt av.

**Produkt-roadmap sist oppdatert:** 2026-08-23
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er implementert og **ACTIVE** i staging som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone incident restore og fail-closed health. Fase 3E.1B.1–3E.1B.2 er implementert med atomisk `reserve_or_get`, root-eid systemd socket/bridge, unik selection-binding og revision-snapshot i PostgreSQL, separat delivery-root, create-only/no-clobber, faktisk bilde-read-back og idempotent activation. Foundationen ble deployet og verifisert mot merge `d756b4b`, og materialisering ble deretter aktivert og verifisert mot merge `ee42c82`. `PUBLIC_IMAGE_RELEASE_MATERIALIZATION_ENABLED=True`, release-count er `1`, og safety-ledgeren er `READY` på cursor `5`.

Fase 3E.1C-koden legger til streng canonical `GET`/`HEAD`-route, eksakt DB-/publication-/tre-filers gate, read-only safety-`authorize`, writer-preferred lifecycle-synkronisering, intern Nginx `X-Accel-Redirect`, read-only deliverymount, eksplisitte allowlistede origins, private 60-sekunders revalidation og strukturerte utfallslogger. Kode- og eksempelstandard er `PUBLIC_IMAGE_SERVING_ENABLED=False`; staging er ikke aktivert før separat livegate. Projection, public API/HTML, legacy-cutover, takedown og 3E.2–3E.4 er uendret. Se [foundationrapporten](STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md) og [aktiveringsrapporten](STAGING_PHASE_3E1B_MATERIALIZATION_ACTIVATION_2026-08-23.md).

Neste gate er separat stagingdeploy og aktivering av 3E.1C med PUBLIC fortsatt urørt. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R, 3B.3, 3B.3-A og 3C.7 er gjennomført. Host-persistente interne stagingpaths er aktivert og restore-verifisert. ADR-009s 3E.1A og 3E.1B-materialisering er **ACTIVE** i staging. Separat public delivery-root og bridge er live backup-/restore-verifisert, og én permanent syntetisk release beviser materialiseringsworkflowen. Serving/origins er implementert i 3E.1C bak default-off gate; projection, API, PUBLIC og takedown er fortsatt ikke implementert. Endelig purge, fallbackinnhold og videre observability tas i 3E.2–3E.4.

Godkjent fase 3B.2-kontrakt skiller intern artifact identity fra public release identity. ADR-009 plasserer artifacts og public releases i separate roots og velger kontrollert serving fra lokal host-persistent storage. Private originaler og artifacts skal forbli private. Hybridbackup inkluderer aktive rendition-bytes, mens ledger/deny-state har fått separat failure-domain og live-verifisert fail-closed restore-gate i 3E.1A. Delivery-/materialiseringsimplementasjonen er aktiv og restore-verifisert i 3E.1B; servingimplementasjonen er kodeklar i 3E.1C, mens livegate og full katastrofe-RTO gjenstår. Backup-RPO og restore-smoke er foreløpig målt. Ekstern S3/CDN er utsatt.

Godkjent fase 3B.3-kontrakt gjør public release identity til en separat, varig reservert UUIDv4 og bruker canonical relative keys `releases/<release_uuid>/<variant>.<ext>`. R1 → R2 kan bruke identiske `ImageRendition`-bytes uten ny encoding, men R2 får alltid ny UUID og nye keys. Fase 3B.3-A bevarer mappingen til tenant, Organization, selection, rendition-sett og artifact gjennom immutable relasjoner og snapshots uten å eksponere disse identitetene i keyen. Hver public key genereres internt og valideres mot eksakt builder-resultat for release-ID, variant og outputformat; caller kan ikke levere fri key. ADR-009s reservation-, binding- og materialiseringsgater er implementert i 3E.1B, mens serving/projection/API/PUBLIC/takedown fortsatt gjenstår.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger, off-server anchor og restore-gate er **ACTIVE** i staging. `image_renditions_public` er fortsatt bare artifact-storage. 3E.1B-materialisering er aktiv og bevist med én permanent syntetisk release og tre delivery-filer. 3E.1C-controlled serving/origins er kodeklar bak separat default-off gate. Neste gate er 3E.1C-stagingaktivering, ikke projection, API/PUBLIC-cutover eller takedown.
