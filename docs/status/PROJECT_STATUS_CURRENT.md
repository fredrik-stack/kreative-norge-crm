...
**Status:** Fase 1 og 2 gjennomført; fase 3A–3D.2 gjennomført som tidligere dokumentert; lokal Hetzner storage-/backup-MVP og ADR-009 fase 3E.1A safety-ledger/off-server anchor er **ACTIVE** i staging; fase 3E.1B.1–3E.1B.2 er implementert i kode som feature-avslått public-delivery-, bridge-, reservation-, DB-binding-, materialiserings-, verifikasjons- og activationgrunnmur, men er **IKKE AKTIVERT I STAGING**; fase 3E.1C–3E.4 og all public serving/runtime er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere; ingen offentlig bildebruk

**Teknisk sist verifisert:** 2026-08-22
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er implementert og **ACTIVE** i staging som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone incident restore og fail-closed health. Fase 3E.1B.1–3E.1B.2 er nå implementert i kode med atomisk `reserve_or_get`, root-eid systemd socket/bridge, unik selection-binding og revision-snapshot i PostgreSQL, separat delivery-root, create-only/no-clobber, faktisk bilde-read-back og idempotent activation. `PUBLIC_IMAGE_RELEASE_MATERIALIZATION_ENABLED` er `False` som kode- og eksempelstandard. Foundationen er ikke deployet, backup-/restore-verifisert eller aktivert i staging, og ingen public runtime er aktivert.

Neste gate er separat deployment-/stagingverifikasjon av 3E.1B-foundationen med flagget fortsatt av: migrasjonens tomtabellkrav, systemd socket activation, faktisk `SO_PEERCRED` gjennom Docker-bindmount, timeout mot Gunicorn, delivery-persistens og backup/restore må bevises før aktivering. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R, 3B.3, 3B.3-A og 3C.7 er gjennomført. Host-persistente interne stagingpaths er aktivert og restore-verifisert. ADR-009s 3E.1A er **ACTIVE** i staging. Separat public delivery-root og materialisering er implementert i 3E.1B-koden, men ikke stagingaktivert eller live restore-verifisert. Serving/origins, projection, API, PUBLIC og takedown er fortsatt ikke implementert. Cache/purge, fallbackinnhold og videre observability tas i 3E.1C–3E.4.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger, off-server anchor og restore-gate er **ACTIVE** i staging. `image_renditions_public` er fortsatt bare artifact-storage. 3E.1B har i kode lagt separat public delivery-root og canonical `releases/...`-materialisering, men flagget er av og foundationen er ikke deployet eller live-verifisert. Controlled serving, projection, API/PUBLIC-cutover, cache/purge og takedown er ikke implementert. Neste gate er 3E.1B stagingforberedelse, ikke PUBLIC-aktivering.
