...
**Status:** Fase 1 og 2 gjennomført; fase 3A, fase 3B.1 og fase 3B.2 teknisk gjennomført; fase 3B.1R og fase 3B.3 gjennomført og godkjent; fase 3B.3-A har additiv public release-domenegrunnmur; lokal Hetzner storage-/backup-MVP **ACTIVE**; fase 3C har bilde-/selection- og processinggrunnmur; fase 3D.1 er teknisk aktivert og visuelt godkjent i staging; fase 3D.2 med precision/zoom er gjennomført og merget til `main` med PR #33; ADR-009 fase 3E.1A safety-ledger/restore/health er implementert i repoet og ekstern staging-anchor er `PREPARED / MANUAL REQUIRED`; fase 3E.1B–3E.4 og public runtime er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere før sluttbrukeravtalegaten er oppfylt; ingen offentlig bildebruk

**Teknisk sist verifisert:** 2026-08-17

**Teknisk verifisert mot:** fase 2-applikasjonsversjonen i merge-commit `6768af8a3b48314aec028ec5972939c6ef0e38e8`, senere additive fase 3-leveranser, stagingbranchens runtimecommit `17919df0d8778ad2600914d4459415466bfcf8e2`, 3D.2-baseline `34f6f35287eaf461d73d40bed25e26f2d4f4b198`, precision/zoom-runtimecommit `3686f08006a1396fd1e2ce250603044c4b62e041` og PR #33s mergecommit `48f23f183dacb8331a64b86f1d7574250cbfbe02` på `main`. 3E.1A er utviklet fra verifisert `origin/main` `9f1159d9cb237520986585e487bbef45fe68c4ad`; pre-activation-gaten er lokalt grønn med 27 safety-ledgertester native og i Linux-container, 372 backendtester på ren migrert testdatabase og isolert test-mediaområde, 4 stagingkontrakttester, 68 backuptester og ShellCheck. Borg-versjonsvinduet er eksakt `>=1.2.8,<1.3.0`, og manuell staginggate omfatter separat subaccount-passordcustody samt delete-/compact-/raw-rm-/recoveryprobe. Den opprinnelig etterspurte CI-evidensen er fortsatt seks grønne jobber i run `32069227961` på head `905ef93f1d17ef82d6bf12902383d8b2e6d66780`. Siste restore-gate utvider dette med eksplisitt clean/incident restore, separat autoritativ cursor/head ved incident og krav om forskjellige safety-/ADR-008-repository-ID-er før destructive probe; alle seks jobber i run `32071145941` er grønne på implementasjonscommit `58273dbff33def39280e428ae64e5c7ab431110e`. Tidligere full 3E.1A-verifisering omfatter også 28 frontendtester, systemd-analyze, installerpermissiontest, frontendbuild og begge produksjonscontainerbyggene. Ekstern safety-anchor er ikke del av lokal/CI-evidens og er fortsatt manuell. Historisk 3D.2-evidens og stagingtilstand er ellers uendret: API/PUBLIC/legacybildekontrakten er fortsatt den gamle, og ingen public release eller mediafil er opprettet. Se [3E.1A-forberedelsen](STAGING_PHASE_3E1A_PREPARATION_2026-08-17.md), [3D.2-stagingverifiseringen 2026-08-11](STAGING_IMAGE_SOURCES_2026-08-11.md), [stagingaktiveringen 2026-08-10](STAGING_IMAGE_RUNTIME_ACTIVATION_2026-08-10.md) og [backupaktiveringen 2026-08-02](STAGING_BACKUP_ACTIVATION_2026-08-02.md).

**Produkt-roadmap sist oppdatert:** 2026-08-17
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er nå implementert i repoet som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone restore og fail-closed health. Host/systemd-oppsettet bruker planlagt `/var/lib/kreative-norge-image-safety/ledger.sqlite3`; API/web får ingen mount, Borg eller credential. Dedikert Storage Box-subaccount/repository, live append/delete/recoveryevidens og stagingrestart er fortsatt `PREPARED / MANUAL REQUIRED`, så ingen public runtime er aktivert.

3E.1A-koden og syntetisk restorebevis er levert; neste operasjonelle gate er prosjekteiers dedikerte off-serverressurs og live capability-/restoreøvelse. Deretter er neste kodegate 3E.1B, ikke PUBLIC-aktivering. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4 har bevist release-deny, tenant-scopet checksum-deny, legacyguard, felles projection/gateway-state, originblokkering, cache expiry/purge/verifikasjon og republisering med ny UUID/key.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R med representativ kvalitet og sRGB, fase 3B.3 release identity-/key-kontrakten, fase 3B.3-A-domenegrunnmuren og fase 3C.7s interne processing/storage er gjennomført. Host-persistente stagingpaths er aktivert og restore-verifisert. ADR-009s 3E.1A-ledger/read-model/restore-/healthkode er implementert, mens dedikert off-server stagingkjede venter på manuell ekstern aktivering. Separat public delivery-root, materialisering, serving/origins, projection, API, PUBLIC og takedown er fortsatt ikke implementert. Cache/purge, fallbackinnhold og videre observability tas i 3E.1C–3E.4. SVG er fortsatt avvist; eventuell sikker rasterisering er en senere behovsdrevet gate. S3, CDN, ekstern IAM, bucket-policy, KMS, Object Lock og provider-spesifikk `versionId`-/purgeverifikasjon er bare en betinget senere gate dersom objektlagring tas opp igjen på grunn av dokumentert behov.

Godkjent fase 3B.2-kontrakt skiller intern artifact identity fra public release identity. ADR-009 plasserer artifacts og public releases i separate roots og velger kontrollert serving fra lokal host-persistent storage. Private originaler og artifacts skal forbli private. Hybridbackup inkluderer aktive rendition-bytes, mens ledger/deny-state får separat failure-domain og alltid avstemmes fail-closed før public serving etter restore. Implementasjon og evidens gjenstår; backup-RPO og restore-smoke er foreløpig målt, mens full katastrofe-RTO fortsatt er åpen. Ekstern S3/CDN er utsatt.

Godkjent fase 3B.3-kontrakt gjør public release identity til en separat, varig reservert UUIDv4 og bruker canonical relative keys `releases/<release_uuid>/<variant>.<ext>`. R1 → R2 kan bruke identiske `ImageRendition`-bytes uten ny encoding, men R2 får alltid ny UUID og nye keys. Fase 3B.3-A bevarer mappingen til tenant, Organization, selection, rendition-sett og artifact gjennom immutable relasjoner og snapshots uten å eksponere disse identitetene i keyen. Hver public key genereres internt og valideres mot eksakt builder-resultat for release-ID, variant og outputformat; caller kan ikke levere fri key. ADR-009 formaliserer de separate runtimegatene; de er fortsatt uimplementert.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C har additive bilde-/selection-modeller og atomiske kommandoer for locking/replacement, ordinær fjerning til fallback og restore som ny revisjon. Fase 3C.7 legger til feature-gated ingest, processing, privat original-storage og intern artifact-storage med immutable tenant-scopede keys, eksakt no-clobber, post-write-verifikasjon og atomisk/idempotent databaseaggregate. Fase 3D.1 er teknisk aktivert og visuelt godkjent i staging med host-persistent storage, verifisert backup/restore og minimal orphan-cleanup. Fase 3D.2 med precision/zoom er gjennomført og merget med PR #33, CI-grønn på `main`, historisk live Brave-verifisert og visuelt eiergodkjent i staging; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger er implementert lokalt og CI-gated, men ekstern staging-anchor er `MANUAL REQUIRED`. `image_renditions_public` er fortsatt bare artifact-storage; separat public delivery-root, canonical `releases/...`-materialisering, controlled serving, projection, API/PUBLIC-cutover, cache/purge og takedown er ikke implementert. Neste kodegate etter off-serverbeviset er 3E.1B, ikke PUBLIC-aktivering.
...
- ADR-009s gjenstående 3E.1A-driftsevidens: dedikert Storage Box-subaccount/repository, live writer/delete/overwrite/recoverytest, stagingrestart og full katastrofe-RTO; schema, writer, hostpath, execution placement og syntetisk restore er implementert
- ADR-009s 3E.1C/3E.4-evidens: konkret cache-TTL/headers, purge-/verifikasjonskontrakt, fallbackgrafikk/-alttekst, retensjon, sync/async og observability; S3/CDN tas bare opp igjen ved dokumentert behov
