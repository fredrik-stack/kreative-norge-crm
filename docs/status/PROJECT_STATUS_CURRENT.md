...
[ADR-008](../decisions/ADR-008-HETZNER_ONE_SERVER_STORAGE_AND_BACKUP_BASELINE.md) velger en enklere operasjonell MVP: app, database og aktiv media på dagens Hetzner Cloud-server, lokale navngitte Django-storagealiaser og kryptert Borg-backup til separat Hetzner Storage Box. S3/AWS/Backblaze/CDN utsettes. Repoets scripts, systemd-maler, tester og runbook er PREPARED. Modulen har en eksplisitt, ikke-overskrivende eksport av den krypterte Borg-repositorynøkkelen og en skrivebeskyttet repository-inspeksjon som begge bruker samme root-only Borg-/SSH-/repository-ID-kontrakt som backupjobben. Den [skrivebeskyttede serverbaselinen](STAGING_BACKUP_BASELINE_2026-08-01.md) er gjennomført, og Cloud Backups er **ENABLED AND FIRST BACKUP VERIFIED** med første synlige backup på 11,6 GB. Storage Box, første Borg-backup, restore-smoke, recovery-secret, eksportert repositorynøkkel og Storage Box-snapshots er fortsatt ikke opprettet eller verifisert; systemd-timerne er ikke installert eller aktivert.
