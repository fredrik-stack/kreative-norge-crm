...
**Status:** Fase 1 og 2 gjennomført; fase 3A, fase 3B.1 og fase 3B.2 teknisk gjennomført; lokal Hetzner storage-/backup-MVP ACTIVE; fase 3C har avslått konfigurasjon og additiv bilde-, selection- og event-/kommandogrunnmur uten runtimebruk

**Teknisk sist verifisert:** 2026-08-03

**Teknisk verifisert mot:** fase 2-applikasjonsversjonen i merge-commit `6768af8a3b48314aec028ec5972939c6ef0e38e8`, fortsatt kjørende API-/web-images, PostgreSQL, Django, migrasjoner, HTTPS, PUBLIC API/HTML, Editor-API og kontrollert tenant-avgrenset telefonreparasjon etter verifisert backup; fase 3B.1 og fase 3B.2 er i tillegg verifisert som isolerte lokale/Linux-prototyper uten CRM-runtimekobling. Første fase 3C-konfigurasjonsgrunnmur er verifisert med 18 avgrensede settings-tester. Andre leveranse legger additivt til `ImageAsset`, `ImageRenditionSet` og `ImageRendition` med migrasjon `0021`. PR #20s tredje leveranse med `OrganizationImageSelection` og migrasjon `0022` er merget. Fjerde leveranse legger til `ImageReviewEvent`, migrasjon `0023` og én feature-gated selection-kommando med atomisk locking/replacement, 30 målrettede PostgreSQL-tester og en grønn backendpakke på 220 tester. Base manager tillater bare lesing, inserts og nødvendig nullstilling av nullable live-referanser, mens URL-snapshots avviser credentials, fragmenter og sensitive queryparametere. Featureflagget er fortsatt avslått; ingen API-, fil-, storage-, bildebehandlings- eller runtimeflyt kaller kommandoen. Staging-repoet er separat fast-forwardet og rent på backupmodulens godkjente `main`-commit uten deploy eller containerrestart. [Backupaktiveringen 2026-08-02](STAGING_BACKUP_ACTIVATION_2026-08-02.md) verifiserte hele Storage Box-/Borg-kjeden, recovery-custody, første backup, full repository-check, isolert restore og aktive timere.
...
Asset-/renditiongrunnmuren fra PR #19 og `OrganizationImageSelection`-skjemaet fra PR #20 er merget. Fjerde leveranse implementerer `ImageReviewEvent` som append-only på applikasjons-/ORM-nivå og `lock_organization_image_selection` som eneste godkjente selection-skriverute. Kommandoen bruker `Organization`-raden som concurrency-lås, krever `expected_revision`, håndhever tenant og capability og skriver første låsing eller replacement atomisk med eventet. Dette er ikke database-WORM. Featureflagget er fortsatt avslått, og ingen API-, Editor-, PUBLIC-, import- eller annen applikasjonsflyt kaller kommandoen. Det finnes fortsatt ingen host-mediaområder, filer, serving eller deploy. Eksisterende default-storage og dagens eksterne `thumbnail_image_url`-, `auto_thumbnail_url`-, `og_image_url`- og faviconflyt gjelder fortsatt. Proveniens lagres som event-snapshot ved asset-selection, men kandidat-/ingestmodellen er ikke implementert. Neste fase 3C-leveranse krever separat godkjenning.
...
Fase 3A-kartleggingen, ADR-007 og de isolerte prototypene i fase 3B.1 og fase 3B.2 er gjennomført. Prosjekteier har godkjent tilhørende processing-, storage-, delivery-, takedown- og restoreprinsipper, og ADR-008s lokale Hetzner-MVP er ACTIVE. Fase 3C har avslått featureflag, ubrukte lokale image-storagealiaser, additive bilde-/selection-modeller og en ubrukt, atomisk selection-kommando med append-only locking-/replacement-event. Filskriving, øvrig eventhistorikk, serving og offentlig bildebruk er fortsatt ikke implementert; fase 3B.1R og øvrige kvalitetsgater må være grønne før reell bildebruk kan aktiveres.
