...
**Status:** Fase 1 og 2 gjennomført; fase 3A, fase 3B.1 og fase 3B.2 teknisk gjennomført; fase 3B.1R og fase 3B.3 gjennomført og godkjent; fase 3B.3-A har additiv public release-domenegrunnmur; lokal Hetzner storage-/backup-MVP **ACTIVE**; fase 3C har bilde-/selection- og processinggrunnmur; fase 3D.1 er teknisk aktivert og visuelt godkjent i staging; fase 3D.2 med precision/zoom er gjennomført og merget til `main` med PR #33, fullverifisert lokalt, CI-grønn, historisk live Brave-verifisert og visuelt eiergodkjent i staging; ADR-009 har godkjent fase 3E public image runtime-arkitektur, men runtimeen er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere før sluttbrukeravtalegaten er oppfylt; ingen offentlig bildebruk

**Teknisk sist verifisert:** 2026-08-13

**Teknisk verifisert mot:** fase 2-applikasjonsversjonen i merge-commit `6768af8a3b48314aec028ec5972939c6ef0e38e8`, senere additive fase 3-leveranser, stagingbranchens runtimecommit `17919df0d8778ad2600914d4459415466bfcf8e2`, 3D.2-baseline `34f6f35287eaf461d73d40bed25e26f2d4f4b198`, precision/zoom-runtimecommit `3686f08006a1396fd1e2ce250603044c4b62e041` og PR #33s mergecommit `48f23f183dacb8331a64b86f1d7574250cbfbe02` på `main`. Lokal fullverifisering før merge er grønn med 372 backendtester på ren migrert testdatabase, 28 frontendtester, 11 Playwright-reiser, 4 stagingkontrakttester, 68 backuptester, frontendbuild og begge produksjonscontainerbyggene. Alle fem jobber i main-CI-run `31535260891` er grønne på eksakt mergecommit. Staging har anvendt 0028 og behandlet en faktisk Foto-oppskrift med X/Y `0.3700/0.6800`, zoom `1.5000` og tre private no-store-previews uten selection-, event-, publiserings- eller releaseendring. Den historiske live-gaten sendte den foreslåtte queryen `Festspillene Helgeland VEFSN` med fast `search_lang=nb`, mottok 30 kandidater og gjennomførte privat originalpreview, secure fetch, processing og tre private no-store-renditionpreviews. Den la bare til ett privat asset, ett immutable rendition-sett og tre interne renditions; selection-, review-event-, publiserings- og public-releaseantall var uendret, med fortsatt 0 public releases. Prosjekteier godkjente samtidig den visuelle precision/zoom-, Foto/Logo-, blank-alt- og live/server-previewreisen. Etterpå ble credentialen med hensikt deaktivert; staging rapporterer nå `brave_configured=False` og kontrollert `brave_not_configured`. PUBLIC, public serving og legacybildene er uendret. Se [3D.2-stagingverifiseringen 2026-08-11](STAGING_IMAGE_SOURCES_2026-08-11.md), [stagingaktiveringen 2026-08-10](STAGING_IMAGE_RUNTIME_ACTIVATION_2026-08-10.md) og [backupaktiveringen 2026-08-02](STAGING_BACKUP_ACTIVATION_2026-08-02.md).
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Den beslutter lokal append-only SQLite-ledger med restore-sikkert WORM-orientert off-server anker, separat `/srv/kreative-norge/media/public-delivery/`, permanent reservasjon før materialisering, create-only/no-clobber, Django-gate med intern Nginx-serving, eksplisitte origins, én `PublicImageProjection`, strukturert `image`-API og trinnene 3E.1A–3E.4. Dette er beslutning, ikke implementering: journal, anchor, delivery-root, materialisering, serving, projection, API/PUBLIC-cutover og takedown finnes fortsatt ikke.

Første implementeringsgate er 3E.1A. Den skal bevise credential-/executionmodellen og at nyere reservation-/deny-state i separat failure-domain vinner over eldre DB/apprestore. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane/root. Formell takedown forblir deaktivert frem til 3E.4 har bevist release-deny, tenant-scopet checksum-deny, legacyguard, felles projection/gateway-state, originblokkering, cache expiry/purge/verifikasjon og republisering med ny UUID/key.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R med representativ kvalitet og sRGB, fase 3B.3 release identity-/key-kontrakten, fase 3B.3-A-domenegrunnmuren og fase 3C.7s interne processing/storage er gjennomført. Host-persistente stagingpaths er aktivert og restore-verifisert. ADR-009 har besluttet kontrakten for journal, separat public delivery-root, materialisering, serving/origins, projection, API og takedown; den er ikke implementert. Konkrete evidensvalg for ledger/anchor, cache/purge, fallbackinnhold, sync/async og observability tas i 3E.1A–3E.4. SVG er fortsatt avvist; eventuell sikker rasterisering er en senere behovsdrevet gate. S3, CDN, ekstern IAM, bucket-policy, KMS, Object Lock og provider-spesifikk `versionId`-/purgeverifikasjon er bare en betinget senere gate dersom objektlagring tas opp igjen på grunn av dokumentert behov.
...
- ADR-009s 3E.1A-evidens: eksakt SQLite-schema/writer/path, off-server ankermekanisme, credential-/execution placement, append-/recoverybevis og full katastrofe-RTO
