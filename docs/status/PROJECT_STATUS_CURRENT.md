...
**Status:** Fase 1 og 2 gjennomført; fase 3A, fase 3B.1 og fase 3B.2 teknisk gjennomført; fase 3B.1R og fase 3B.3 gjennomført og godkjent; fase 3B.3-A har additiv public release-domenegrunnmur; lokal Hetzner storage-/backup-MVP **ACTIVE**; fase 3C har bilde-/selection- og processinggrunnmur; fase 3D.1 er teknisk aktivert og visuelt godkjent i staging; fase 3D.2 med precision/zoom er gjennomført og merget til `main` med PR #33; ADR-009 fase 3E.1A safety-ledger, off-server anchor og restore-gate er **ACTIVE** i staging; 3E.1B-kontrakten er godkjent, men fase 3E.1B–3E.4 og public runtime er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere før sluttbrukeravtalegaten er oppfylt; ingen offentlig bildebruk

**Teknisk sist verifisert:** 2026-08-20
...
**Produkt-roadmap sist oppdatert:** 2026-08-20
...
Fase 3B.3 er **GJENNOMFØRT / GODKJENT** som arkitektur- og kontraktgate. [ADR-007 punkt 25](../decisions/ADR-007-IMAGE_ASSET_ARCHITECTURE.md#25-fase-3b3-godkjent-public-release-identity-og-key-kontrakt) fastsetter tilfeldig UUIDv4 som separat immutable public release identity og relative canonical keys på formen `releases/<release_uuid>/<variant>.<ext>`. Selection-revisjon, tenant-/Organization-identitet og artifact-hash inngår ikke i keyen. Replacement, restore og senere autorisert republisering går via ny selection-revisjon og får alltid ny release-ID, mens retry av samme selection-revisjon gjenbruker samme reservation/UUID/keys og tidligere ID-er/keys forblir varig reservert. Fase 3B.3-A implementerer de valgte modellene, immutable relasjoner og snapshots, manager-/modellbeskyttelse, PostgreSQL-constraints og domenetjenesteinvarianter. Fase 3E.1A har senere implementert og stagingaktivert permanent reservation-/lifecycle-ledger med dedikert off-server anchor; DB-/filmaterialisering, storage-runtime og projection er ikke implementert.
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er implementert og **ACTIVE** i staging som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone incident restore og fail-closed health. Host/systemd-oppsettet bruker `/var/lib/kreative-norge-image-safety/ledger.sqlite3`; API/web får ingen safety-mount, Borg eller credential. Dedikert Storage Box-subaccount/repository, separat recovery-custody, capability-/transaction-recoveryevidens og stagingrestart er live-verifisert. 3E.1B er nå arkitektonisk presisert med lokal Unix-socket/systemd-bro, maksimalt én release per selection-revisjon, ledger-autoritativ lifecycle og ingen automatisk releasefilsletting. Dette er besluttet, men ikke implementert; ingen public runtime er aktivert.

3E.1A-koden og den eksterne restore-gaten er levert; neste kodegate er 3E.1B.1 for bro, idempotent reservasjon og DB-binding, ikke PUBLIC-aktivering. Dagens release-tjeneste genererer fortsatt ny UUID per kall, dagens PostgreSQL-modell har ingen unik selection-binding, og 3B.3-A-regresjonstesten forventer to ulike releases fra samme selection. Denne verifiserte pre-3E-adferden må endres, og den konkrete idempotency-håndhevingen må avgjøres i implementasjonen uten en parallell lifecycle-status. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Writerens verifiserte raw-`rm`-capability er akseptert restrisiko, mens separat recovery-custody er den operative gjenopprettingsgrensen. Formell takedown forblir deaktivert frem til 3E.4 har bevist release-deny, tenant-scopet checksum-deny, legacyguard, felles projection/gateway-state, originblokkering, cache expiry/purge/verifikasjon og republisering med ny UUID/key.
