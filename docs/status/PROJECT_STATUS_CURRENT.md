...
**Produkt-roadmap sist oppdatert:** 2026-08-10
...
### 3. Thumbnail-, bilde- og kortarkitektur – fase 3D.1 merget bak avslått feature

Fase 3A-kartleggingen, ADR-007 og de isolerte prototypene i fase 3B.1 og fase 3B.2 er gjennomført. Fase 3B.1R har i tillegg gjennomført representativ lokal kvalitetsevidens og fått godkjent decoded pixel-, dimensjons/no-upscale-, advisory kvalitets- og sRGB-kontrakt. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C har avslått featureflag, additive bilde-/selection-modeller og atomiske kommandoer for locking/replacement, ordinær fjerning til fallback og restore som ny revisjon. Fase 3C.7 legger til eksplisitt, feature-gated upload-only ingest, processing, privat original-storage og intern artifact-storage med immutable tenant-scopede keys, eksakt no-clobber, post-write-verifikasjon og atomisk/idempotent databaseaggregate. Fase 3D.1 kaller denne tjenesten fra den interne offisielle kandidatflyten i API og Editor når feature eksplisitt aktiveres, men flagget er fortsatt avslått som default og er ikke aktivert i staging. `image_renditions_public` er ikke offentlig serving. Editor-upload, PUBLIC, canonical `releases/...`-materialisering, permanent reservation-/deny-journal, cache/purge, host-persistent staging-media, retention/orphan-cleanup, staging og deploy er fortsatt ikke implementert eller aktivert.
