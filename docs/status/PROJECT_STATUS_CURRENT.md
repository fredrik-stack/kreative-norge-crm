...
**Status:** Fase 1 og 2 gjennomført; fase 3A–3D.2 gjennomført som tidligere dokumentert; lokal Hetzner storage-/backup-MVP og ADR-009 fase 3E.1A safety-ledger/off-server anchor er **ACTIVE** i staging; fase 3E.1B-materialisering er **ACTIVE / VERIFIED**; fase 3E.1C-controlled serving er **CLOSED / ACTIVE**; fase 3E.2 projection/API shadow er **CLOSED / SHADOW VERIFIED** med projection aktiv og schema av i staging; fase 3E.3 er **IMPLEMENTED / STAGING GATE PENDING** bak default-off schema-/PUBLIC-gater; fase 3E.4 er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere; staging-PUBLIC bruker fortsatt legacybilder

**Teknisk sist verifisert:** 2026-08-24
...
3E.3 er nå implementert i kode bak `PUBLIC_IMAGE_API_SCHEMA_ENABLED=False` og den nye globale fail-closed-gaten `PUBLIC_IMAGE_PUBLIC_CUTOVER_ENABLED=False`. PUBLIC list/detail, target-API, canonical og Open Graph/Twitter bruker samme projection når gatene aktiveres; production fallback v1 gjenbruker de eksisterende visuelt kontrollerte bytene på canonical static-stier og har blank alttekst. Host/proxyinvarians, privacy, safety-fallback, fixed-query-prefetch, browserfallback og desktop/mobil er testet lokalt. Ingen modell eller migrasjon er lagt til, og tenant-enrollment er ikke innført uten konkret blocker. Dette er implementeringsstatus, ikke stagingevidens: targetschema og PUBLIC forblir av i staging frem til separat deploy-, baseline-, aktiverings-, rollback- og visuell gate.

**Produkt-roadmap sist oppdatert:** 2026-08-24
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er implementert og **ACTIVE** i staging som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone incident restore og fail-closed health. Fase 3E.1B.1–3E.1B.2 er implementert med atomisk `reserve_or_get`, root-eid systemd socket/bridge, unik selection-binding og revision-snapshot i PostgreSQL, separat delivery-root, create-only/no-clobber, faktisk bilde-read-back og idempotent activation. Foundationen ble deployet og verifisert mot merge `d756b4b`, materialisering ble aktivert mot merge `ee42c82`, og serving ble aktivert mot merge `38663b5`. Materialiserings- og servingflaggene er `True` bare i staging; release-count er `2`, og safety-ledgeren er `READY` på cursor `7`.

Fase 3E.1C legger til streng canonical `GET`/`HEAD`-route, eksakt DB-/publication-/tre-filers gate, read-only safety-`authorize`, writer-preferred lifecycle-synkronisering, intern Nginx `X-Accel-Redirect`, read-only deliverymount, eksplisitte allowlistede origins, private 60-sekunders revalidation og strukturerte utfallslogger. Kode- og eksempelstandard er fortsatt `PUBLIC_IMAGE_SERVING_ENABLED=False`, mens ignorert stagingverdi er `True`. 3E.2 gjenbruker samme releasevalidering og safety-autorisasjon i read-only projection og er `CLOSED / SHADOW VERIFIED`. 3E.3-koden er implementert, men targetschema og public API/HTML/head-cutover forblir av i staging til separat gate. Takedown og 3E.4 er ikke implementert. Se [3E.1B-foundationen](STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md), [3E.1B-aktiveringen](STAGING_PHASE_3E1B_MATERIALIZATION_ACTIVATION_2026-08-23.md), [3E.1C-aktiveringen](STAGING_PHASE_3E1C_ACTIVATION_2026-08-24.md) og [3E.2-shadowgaten](STAGING_PHASE_3E2_SHADOW_2026-08-24.md).

Neste gate er implementerings-PR/review/CI og deretter separat 3E.3-stagingaktivering. Targetschemaet skal aktiveres og rollbacktestes før PUBLIC-cutoveren slås på; begge skal bruke eksakt implementation merge. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R, 3B.3, 3B.3-A og 3C.7 er gjennomført. Host-persistente interne stagingpaths er aktivert og restore-verifisert. ADR-009s 3E.1A, 3E.1B-materialisering og 3E.1C-serving er **ACTIVE** i staging. Separat public delivery-root og bridge er live backup-/restore-verifisert, og to releases med seks filer beviser materialisering og controlled serving. Projection/API-shadow er `CLOSED / SHADOW VERIFIED`; schema/PUBLIC-cutover er implementert, men staginggaten gjenstår. Production fallback v1 og blank alttekst er valgt i 3E.3. Endelig purge, takedown og eksterne alertterskler tas i 3E.4 eller ved senere dokumentert behov.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger/off-server anchor, 3E.1B-materialisering og 3E.1C-controlled serving er **ACTIVE** i staging, og 3E.2 er **CLOSED / SHADOW VERIFIED**. 3E.3 er implementert med default-off gater og venter på PR-/CI-/staginggaten. `image_renditions_public` er fortsatt bare artifact-storage. Staging har to release-aggregater og seks delivery-filer. Neste separate driftsgate er kontrollert 3E.3-aktivering, ikke takedown eller 3E.4.
...
- ADR-009s gjenstående 3E.4-/driftsvalg: purge-/verifikasjonskontrakt, retensjon, sync/async og eksterne alertterskler; 3E.3 har valgt global cutovergate og production fallback v1 med blank alttekst, mens S3/CDN bare tas opp igjen ved dokumentert behov
