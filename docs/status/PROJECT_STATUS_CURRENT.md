...
Asset-/renditiongrunnmuren fra PR #19, `OrganizationImageSelection`-skjemaet fra PR #20 og PR #21s locking-/replacement-kjerne er merget. Fase 3C.5 implementerer `remove_organization_image_to_fallback` som eneste godkjente vei fra en aktiv asset-selection til en ny aktiv systemfallback-revisjon med eventtypen `selection_removed_to_fallback`. Begge offentlige selection-kommandoer bruker samme tenant-scopede `Organization`-lås, capabilitymatrise, `expected_revision` og atomiske eventskriving; den generiske lock-kommandoen avviser nå både asset → fallback og fallback → fallback. Eventhistorikken er append-only på applikasjons-/ORM-nivå, ikke database-WORM, og databasen kan ikke alene bevise typen på forrige selection. Featureflagget er fortsatt avslått, og ingen API-, Editor-, PUBLIC-, import- eller annen applikasjonsflyt kaller kommandoene. Det finnes fortsatt ingen host-mediaområder, filer, serving eller deploy. Eksisterende default-storage og dagens eksterne `thumbnail_image_url`-, `auto_thumbnail_url`-, `og_image_url`- og faviconflyt gjelder fortsatt. Restore, kandidat-/ingestmodellen og øvrig eventhistorikk krever separate beslutninger og leveranser.
...
Fase 3A-kartleggingen, ADR-007 og de isolerte prototypene i fase 3B.1 og fase 3B.2 er gjennomført. Prosjekteier har godkjent tilhørende processing-, storage-, delivery-, takedown- og restoreprinsipper, og ADR-008s lokale Hetzner-MVP er ACTIVE. Fase 3C har avslått featureflag, ubrukte lokale image-storagealiaser, additive bilde-/selection-modeller og ubrukte, atomiske kommandoer for locking/replacement og ordinær fjerning til fallback med append-only events. Filskriving, restore, øvrig eventhistorikk, serving og offentlig bildebruk er fortsatt ikke implementert; fase 3B.1R og øvrige kvalitetsgater må være grønne før reell bildebruk kan aktiveres.
