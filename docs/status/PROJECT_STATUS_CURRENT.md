...
Fase 3F-koden innfører additiv migrasjon `0031`, den default-off typed `ImportImageDecision`-kontrakten og en beskyttet én-til-én-binding til anvendt `ImageReviewEvent`. Vanlig Organization CRUD og import-commit utfører ikke lenger automatisk Open Graph-/bildenettverk; legacy URL-feltene er API-read-only, et aggregert management-inventar er write-free, og Editor kan hente egne DB-baserte legacykandidater uten automatisk preview, approval eller write. Import uten eksplisitt beslutning bevarer eksisterende selection; typed apply avviser wrong-tenant, stale actor/selection og ufullstendig approval, er idempotent og endrer ingen publiseringsflagg eller public release. Kode-/eksempelstandard er `IMPORT_IMAGE_DECISIONS_ENABLED=False`; separat staging-/restore-/orphanbevis og evidens-PR gjenstår før 3F og fase 3 kan markeres lukket.
