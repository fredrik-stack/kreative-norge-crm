...
**Status:** Fase 1 og 2 gjennomført; fase 3A–3D.2 gjennomført som tidligere dokumentert; lokal Hetzner storage-/backup-MVP og ADR-009 fase 3E.1A safety-ledger/off-server anchor er **ACTIVE** i staging; fase 3E.1B-materialisering er **ACTIVE / VERIFIED**; fase 3E.1C-controlled serving er **CLOSED / ACTIVE** med to release-aggregater og seks delivery-filer; fase 3E.2 projection/API shadow er implementert bak default-off gater og avventer separat stagingverifikasjon; fase 3E.3–3E.4 og PUBLIC-kobling er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere; PUBLIC bruker fortsatt legacybilder
...
3E.2-koden etablerer én canonical public actors-rute/serializer, en read-only `PublicImageProjection`, delt releasevalidering med serving, tre variantvise safety-authorizations, target `image`-schema, aliasinvarianten, teknisk versjonert systemfallback og en skrivebeskyttet fullkatalog-audit. `PUBLIC_IMAGE_PROJECTION_ENABLED=False` og `PUBLIC_IMAGE_API_SCHEMA_ENABLED=False` er fortsatt kode-/eksempelstandard. Ingen migrasjon er lagt til. Før separat liveevidens finnes er projection ikke aktiv i staging og 3E.2 skal ikke omtales som `CLOSED / SHADOW VERIFIED`.
...
Fase 3E.1C legger til streng canonical `GET`/`HEAD`-route, eksakt DB-/publication-/tre-filers gate, read-only safety-`authorize`, writer-preferred lifecycle-synkronisering, intern Nginx `X-Accel-Redirect`, read-only deliverymount, eksplisitte allowlistede origins, private 60-sekunders revalidation og strukturerte utfallslogger. Kode- og eksempelstandard er fortsatt `PUBLIC_IMAGE_SERVING_ENABLED=False`, mens ignorert stagingverdi er `True` etter grønn livegate. Projection, public API/HTML, legacy-cutover, takedown og 3E.2–3E.4 er uendret. Se [3E.1B-foundationen](STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md), [3E.1B-aktiveringen](STAGING_PHASE_3E1B_MATERIALIZATION_ACTIVATION_2026-08-23.md) og [3E.1C-aktiveringen](STAGING_PHASE_3E1C_ACTIVATION_2026-08-24.md).

Neste gate er separat deploy og staging shadow-verifikasjon av 3E.2, først med begge nye gater av og deretter bare projection på. Schema, PUBLIC og 3E.3 skal forbli av. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger/off-server anchor, 3E.1B-materialisering og 3E.1C-controlled serving er **ACTIVE** i staging. `image_renditions_public` er fortsatt bare artifact-storage. Staging har to release-aggregater og seks delivery-filer. Neste gate er 3E.2 projection/API shadow, ikke PUBLIC-cutover eller takedown.
...
- ADR-009s 3E.2–3E.4-valg: projection/API shadow, purge-/verifikasjonskontrakt, fallbackgrafikk/-alttekst, retensjon, sync/async og eksterne alertterskler; 3E.1C cache/headers og requestobservability er liveverifisert, og S3/CDN tas bare opp igjen ved dokumentert behov
