...
**Status:** Fase 1 og 2 gjennomført; fase 3A, fase 3B.1 og fase 3B.2 teknisk gjennomført; fase 3B.1R og fase 3B.3 gjennomført og godkjent; lokal Hetzner storage-/backup-MVP **ACTIVE**; fase 3C har avslått konfigurasjon og additiv bilde-, selection- og event-/kommandogrunnmur for ordinær lock, replacement, fjerning til fallback og restore som ny revisjon, uten public release aggregate eller runtimebruk

**Teknisk sist verifisert:** 2026-08-07

**Teknisk verifisert mot:** fase 2-applikasjonsversjonen i merge-commit `6768af8a3b48314aec028ec5972939c6ef0e38e8`, fortsatt kjørende API-/web-images, PostgreSQL, Django, migrasjoner, HTTPS, PUBLIC API/HTML, Editor-API og kontrollert tenant-avgrenset telefonreparasjon etter verifisert backup; fase 3B.1 og fase 3B.2 er i tillegg verifisert som isolerte lokale/Linux-prototyper uten CRM-runtimekobling. Fase 3B.1R kjørte den isolerte harnessen lokalt mot 24 rettighetsavklarte fixtures og godkjente 36 MP som konfigurerbar decoded-pixel-standard, variantspesifikk no-upscale, advisory kvalitetsmål og profilfri output etter eksplisitt sRGB-normalisering; private kilder og full/visuell evidens forblir Git-ignorert. Første fase 3C-konfigurasjonsgrunnmur er verifisert med 18 avgrensede settings-tester. Andre leveranse legger additivt til `ImageAsset`, `ImageRenditionSet` og `ImageRendition` med migrasjon `0021`. PR #20s tredje leveranse med `OrganizationImageSelection` og migrasjon `0022` er merget. PR #21s fjerde leveranse med `ImageReviewEvent`, migrasjon `0023` og atomisk locking/replacement er merget. PR #22 og fase 3C.5 med `selection_removed_to_fallback`, migrasjon `0024` og `remove_organization_image_to_fallback` er merget. Fase 3C.6 legger additivt til `selection_restored`, migrasjon `0025` og `restore_archived_organization_image_selection`; 54 målrettede PostgreSQL-tester og hele backendpakken på 246 tester er grønne. Restore krever en eksplisitt arkivert asset-selection i samme tenant og organisasjon, oppretter en ny revisjon og kopierer rendition-sett, alt-tekst og offentlig kreditering uten ny approval eller proveniens. Base manager tillater bare lesing, inserts og nødvendig nullstilling av nullable live-referanser, mens URL-snapshots avviser credentials, fragmenter og sensitive queryparametere. Featureflagget er fortsatt avslått; ingen API-, fil-, storage-, bildebehandlings- eller runtimeflyt kaller kommandoene. Staging-repoet er separat fast-forwardet og rent på backupmodulens godkjente `main`-commit uten deploy eller containerrestart. [Backupaktiveringen 2026-08-02](STAGING_BACKUP_ACTIVATION_2026-08-02.md) verifiserte hele Storage Box-/Borg-kjeden, recovery-custody, første backup, full repository-check, isolert restore og aktive timere.

**Produkt-roadmap sist oppdatert:** 2026-08-07
...
Fase 3C skal implementeres additivt og bak avslått feature. Fase 3B.1R med representativ kvalitet og sRGB samt fase 3B.3 release identity-/key-kontrakten er gjennomført og godkjent. Før reell bildebehandling eller public serving kan aktiveres gjenstår implementering av organization-typed public release aggregate og varig reservasjon; lokal private/public-storage og same-origin/media-origin-serving; lokal cache-, purge- og verifikasjonskontrakt; permanent reservation-/deny-journal og materialisert read-model med journalcursor og fail-closed reconciliation; SVG-policy og eventuell sikker rasterisering; eventuell skadevarekontroll og bakgrunnskø; endelig public API-schema og alias-til-variant-mapping; retensjonsmekanisme; sync/async-grense og observability. S3, CDN, ekstern IAM, bucket-policy, KMS, Object Lock og provider-spesifikk `versionId`-/purgeverifikasjon er bare en betinget senere gate dersom objektlagring tas opp igjen på grunn av dokumentert behov.
...
Godkjent fase 3B.3-kontrakt gjør public release identity til en separat, varig reservert UUIDv4 og bruker canonical relative keys `releases/<release_uuid>/<variant>.<ext>`. R1 → R2 kan bruke identiske `ImageRendition`-bytes uten ny encoding, men R2 får alltid ny UUID og nye keys. Release aggregate skal bevare en immutable historisk mapping til tenant, Organization, selection, rendition-sett og artifact uten å eksponere disse identitetene i keyen. Hver public key genereres internt og må være eksakt builder-resultat for release-ID, variant og outputformat; caller kan ikke levere fri key. Eksakt modell- og håndhevingsmekanisme avgjøres i 3B.3-A. Runtime, journalteknologi, serving, purge, unpublish, fallback-key, API og projection er fortsatt separate gater.
...
### 3. Thumbnail-, bilde- og kortarkitektur – fase 3B.1R gjennomført og besluttet

Fase 3A-kartleggingen, ADR-007 og de isolerte prototypene i fase 3B.1 og fase 3B.2 er gjennomført. Fase 3B.1R har i tillegg gjennomført representativ lokal kvalitetsevidens og fått godkjent decoded pixel-, dimensjons/no-upscale-, advisory kvalitets- og sRGB-kontrakt. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C har avslått featureflag, ubrukte lokale image-storagealiaser, additive bilde-/selection-modeller og ubrukte, atomiske kommandoer for locking/replacement, ordinær fjerning til fallback og restore av en eksplisitt arkivert asset-selection som ny revisjon med append-only events. Filskriving, storage-runtime, øvrig eventhistorikk, serving og offentlig bildebruk er fortsatt ikke implementert; øvrige senere 3B-/3C-gater må være grønne før reell bildebruk kan aktiveres.
...
- implementering av godkjent public release aggregate/reservasjon, samt SVG-policy, eventuell sikker rasterisering/skadevarekontroll/bakgrunnskø, API-schema, aliasmapping, retensjonsmekanisme, sync/async-grense og observability i fase 3B
