...
**Status:** Fase 1 og 2 gjennomført; fase 3A–3D.2 gjennomført som tidligere dokumentert; lokal Hetzner storage-/backup-MVP og ADR-009 fase 3E.1A safety-ledger/off-server anchor er **ACTIVE** i staging; fase 3E.1B.1–3E.1B.2-foundationen er **DEPLOYED / VERIFIED** i staging med release-materialisering fortsatt avslått; fase 3E.1C–3E.4 og all public serving/runtime er ikke implementert; Brave er operativt ikke aktiv for ordinære Editor-sluttbrukere; ingen offentlig bildebruk
...
[ADR-009](../decisions/ADR-009-PUBLIC_IMAGE_RUNTIME_RELEASE_DELIVERY_AND_RESTORE_SAFE_DENY_STATE.md) er godkjent som fase 3E-arkitektur. Fase 3E.1A er implementert og **ACTIVE** i staging som en Django-/PostgreSQL-uavhengig SQLite schema v1-ledger med permanent UUID/key-reservasjon, fire lifecycleevents, canonical payload/hashkjede, rebuildbar read-model/cursor, synkron Borg create/read-back, immutable local receipts, standalone incident restore og fail-closed health. Fase 3E.1B.1–3E.1B.2 er implementert med atomisk `reserve_or_get`, root-eid systemd socket/bridge, unik selection-binding og revision-snapshot i PostgreSQL, separat delivery-root, create-only/no-clobber, faktisk bilde-read-back og idempotent activation. Foundationen er deployet og verifisert i staging mot merge `d756b4b`: migrasjon `0029`, socket/peer, mountisolasjon, timeoutkjede, delivery-persistens og backup/restore er grønne. `PUBLIC_IMAGE_RELEASE_MATERIALIZATION_ENABLED` er fortsatt `False`, release-count er `0`, og ingen public runtime er aktivert. Se [3E.1B-stagingrapporten](STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md).

Neste gate er kontrollert 3E.1B-materialiseringsaktivering med syntetiske komplette renditions: faktisk reserve → DB-binding → create-only/read-back → activate, retry/no-clobber og delvis materialisering/restart skal bevises mens public serving fortsatt er av. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3C implementeres additivt bak featuregaten. Fase 3B.1R, 3B.3, 3B.3-A og 3C.7 er gjennomført. Host-persistente interne stagingpaths er aktivert og restore-verifisert. ADR-009s 3E.1A er **ACTIVE** i staging. Separat public delivery-root og bridge er deployet i 3E.1B-foundationen og live backup-/restore-verifisert, men release-materialisering er fortsatt avslått. Serving/origins, projection, API, PUBLIC og takedown er fortsatt ikke implementert. Cache/purge, fallbackinnhold og videre observability tas i 3E.1C–3E.4.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger, off-server anchor og restore-gate er **ACTIVE** i staging. `image_renditions_public` er fortsatt bare artifact-storage. 3E.1B-foundationen har deployet separat public delivery-root, bridge og canonical `releases/...`-materialiseringskode, men flagget er av og ingen release-workflow er kjørt. Controlled serving, projection, API/PUBLIC-cutover, cache/purge og takedown er ikke implementert. Neste gate er 3E.1B-materialiseringsaktivering med syntetisk evidens, ikke PUBLIC-aktivering.
