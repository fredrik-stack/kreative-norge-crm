...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger, off-server anchor og restore-gate er **ACTIVE** i staging. `image_renditions_public` er fortsatt bare artifact-storage. 3E.1B-materialisering er aktiv og bevist med én permanent syntetisk release og tre delivery-filer. Controlled serving, projection, API/PUBLIC-cutover, cache/purge og takedown er ikke implementert. Neste gate er 3E.1C for kontrollert serving og origins, ikke PUBLIC-aktivering.
