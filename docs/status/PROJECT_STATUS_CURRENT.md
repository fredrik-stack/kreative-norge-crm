...
Fase 3E.1C legger til streng canonical `GET`/`HEAD`-route, eksakt DB-/publication-/tre-filers gate, read-only safety-`authorize`, writer-preferred lifecycle-synkronisering, intern Nginx `X-Accel-Redirect`, read-only deliverymount, eksplisitte allowlistede origins, private 60-sekunders revalidation og strukturerte utfallslogger. Kode- og eksempelstandard er fortsatt `PUBLIC_IMAGE_SERVING_ENABLED=False`, mens ignorert stagingverdi er `True`. 3E.2 gjenbruker samme releasevalidering og safety-autorisasjon i read-only projection og er nå `CLOSED / SHADOW VERIFIED`. Target-schema, public API/HTML-cutover, takedown og 3E.3–3E.4 forblir av. Se [3E.1B-foundationen](STAGING_PHASE_3E1B_FOUNDATION_2026-08-23.md), [3E.1B-aktiveringen](STAGING_PHASE_3E1B_MATERIALIZATION_ACTIVATION_2026-08-23.md), [3E.1C-aktiveringen](STAGING_PHASE_3E1C_ACTIVATION_2026-08-24.md) og [3E.2-shadowgaten](STAGING_PHASE_3E2_SHADOW_2026-08-24.md).

Neste gate er separat planlegging og beslutning for 3E.3; den er ikke startet. Target-schema, API/PUBLIC-cutover og endelig fallbackinnhold skal ikke aktiveres implisitt. ADR-009 lover ikke absolutt WORM eller vern mot kompromittert Hetzner control plane, Storage Box main user, host-root eller kombinerte admincredentials. Formell takedown forblir deaktivert frem til 3E.4.
...
Fase 3A-kartleggingen, ADR-007 og fase 3B-grunnlaget er gjennomført. ADR-008s lokale Hetzner-MVP er **ACTIVE**. Fase 3C og 3D.1–3D.2 er gjennomført som tidligere dokumentert; Brave er operativt deaktivert i påvente av sluttbrukeravtalegaten. ADR-009s 3E.1A safety-ledger/off-server anchor, 3E.1B-materialisering og 3E.1C-controlled serving er **ACTIVE** i staging, og 3E.2 er **CLOSED / SHADOW VERIFIED**. `image_renditions_public` er fortsatt bare artifact-storage. Staging har to release-aggregater og seks delivery-filer. Neste separate gate er 3E.3-planlegging, ikke en implisitt PUBLIC-cutover eller takedown.
...
- ADR-009s 3E.3–3E.4-valg: API/PUBLIC-cutover, purge-/verifikasjonskontrakt, fallbackgrafikk/-alttekst, retensjon, sync/async og eksterne alertterskler; 3E.1C cache/headers og 3E.2 projection-shadow er liveverifisert, og S3/CDN tas bare opp igjen ved dokumentert behov
