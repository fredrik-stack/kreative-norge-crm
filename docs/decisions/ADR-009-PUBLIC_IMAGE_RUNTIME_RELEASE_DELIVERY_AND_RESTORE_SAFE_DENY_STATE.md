...
- `private, max-age=60, must-revalidate`, checksum-`ETag`, reautorisert `If-None-Match` → `304`, `no-store` på 404/503, ingen shared proxycache og ingen `immutable`; Nginx auto-ETag er av bare i den interne delivery-locationen slik at checksum-ETag-en ikke erstattes av filmetadata
