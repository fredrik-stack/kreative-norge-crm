...
Operativ safety-kontrakt finnes i `operations/PUBLIC_IMAGE_SAFETY_LEDGER.md`. Safety-ledger, dedikert off-server anchor og restore-gate fra 3E.1A er `ACTIVE` i staging. 3E.1B.1–3E.1B.2-materialisering er også `ACTIVE` etter kontrollert crash/restart/retry/no-clobber-gate. Fase 3E.1C er `CLOSED / ACTIVE` etter separat livegate av kontrollert Django/Nginx-serving. Fase 3E.2 er `CLOSED / SHADOW VERIFIED`: bare projection-shadow er aktiv i staging, mens target-schemaet er av. 3E.3-koden er implementert bak `PUBLIC_IMAGE_PUBLIC_CUTOVER_ENABLED=False` med target-API, PUBLIC, canonical, Open Graph/Twitter og production fallback v1, men er ennå ikke deployet eller aktivert i staging. Staging har fortsatt to release-aggregater og seks delivery-filer, og PUBLIC HTML/API bruker fortsatt legacybilder frem til den separate 3E.3-gaten. Se [3E.2-evidensen](status/STAGING_PHASE_3E2_SHADOW_2026-08-24.md). Formell takedown og fase 3E.4 er ikke implementert.
